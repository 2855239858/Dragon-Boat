...
var AnimationRun_3 = cc.Class({
    ...
    update(dt) {
        ...
        if (game.bar() < 160 && game.bar() > 140 && animState.name != "Animation_5_badSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_5_badSpeed');
        }
        if (game.bar() == 160 && animState.name != "Animation_1_nomalSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_1_nomalSpeed');
        }
        if (game.bar() > 160 && game.bar() < 180 && animState.name != "Animation_2_goodSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_2_goodSpeed');
        }
        if (game.bar() > 180 && game.bar() < 200 && animState.name != "Animation_3_perfectSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
