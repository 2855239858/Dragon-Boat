const ENDLINE = 4200;
var animState, name;
var game = require("npc2");

var AnimationRun2_4 = cc.Class({
    extends: game,
    ...
    update(dt) {
        ...
        if (game.bar() < 130 && game.bar() > 120 && animState.name != "Animation_5_badSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_5_badSpeed');
        }
        if (game.bar() == 120 && animState.name != "Animation_1_nomalSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_1_nomalSpeed');
        }
        if (game.bar() > 120 && game.bar() < 130 && animState.name != "Animation_2_goodSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_2_goodSpeed');
        }
        if (game.bar() > 130 && game.bar() < 140 && animState.name != "Animation_3_perfectSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_3_perfectSpeed');
        }
        if (game.bar() > 140 && animState.name != "Animation_4_comboSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
