...
var AnimationRun2_6 = cc.Class({
    ...
    update(dt) {
        if ((game.bar() < 110) && animState.name != "c_win") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('c_win');
        }
        if (game.bar() < 130 && game.bar() > 110 && animState.name != "Animation_5_badSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_5_badSpeed');
        }
        if (game.bar() > 130 && game.bar() < 140 && animState.name != "Animation_1_nomalSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_1_nomalSpeed');
        }
        ...
        if (game.bar() > 160 && game.bar() < 185 && animState.name != "Animation_3_perfectSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_3_perfectSpeed');
        }
        if (game.bar() > 185&& animState.name != "Animation_4_comboSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
