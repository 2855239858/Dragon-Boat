const ENDLINE = 4200;
var animState, name;
var game = require("npc3");

var AnimationRun2_8 = cc.Class({
    extends: game,
    ...
    update(dt) {
        ...
        if (game.bar() > 150 && game.bar() < 160 && animState.name != "Animation_3_perfectSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_3_perfectSpeed');
        }
        if (game.bar() > 160 && animState.name != "Animation_4_comboSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
