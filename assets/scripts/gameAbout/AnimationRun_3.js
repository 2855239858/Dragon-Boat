...
var AnimationRun_3 = cc.Class({
    ...
    update(dt) {
        ...
        if (game.bar() < 140 && game.bar() > 120 && animState.name != "Animation_5_badSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
        ...
        if (game.bar() > 160 && game.bar() < 180 && animState.name != "Animation_3_perfectSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
            animState = this.animationComponent.play('Animation_3_perfectSpeed');
        }
        if (game.bar() > 180 && animState.name != "Animation_4_comboSpeed") {
            this.animationComponent.stop();
            this.changeOpcity();
